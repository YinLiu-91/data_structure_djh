...
protected:
    ...
    void selectionSort(ListNodePosi(T), int);                            //对从p开始的连续n个节点的选择排序
    void insertionSort(ListNodePosi(T), int);                            //对从p开始连续的n个节点插入排序
public:
    //构造函数
    List() { init(); }                     //默认
    List(List<T> const &L);                //整体复制列表L
    List(List<T> const &L, Rank r, int n); //复制列表中L自第r项开始的n项
    ...
    T &operator[](Rank r) const;                           //重载，支持循秩访问（效率低下）
    ...
    ListNodePosi(T) insertB(ListNodePosi(T) p, T const &e);                   //将e当做p的前驱charu
    ...
    void reverse();                                                           //前后倒置
    //遍历
    void tranverse(void (*)(T &)); //遍历，依次实施visit操作（函数指针，只读或局部性修改）
